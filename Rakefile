...
task :post do
  ...
  if File.exist?(filename) && (ask("#{filename} already exists. Do you want to overwrite?", %w[y n]) == 'n')
    abort('rake aborted!')
  end
...
task :page do
  ...
  title = File.basename(filename, File.extname(filename)).gsub(/[\W_]/, ' ').gsub(/\b\w/) do
...
namespace :theme do
  ...
  task :install do
    ...
    verify_manifest(packaged_theme_path)
    ...
    FileUtils.cd(packaged_theme_path) do
      Dir.glob('**/*.*') do |f|
    ...
    packaged_theme_files.each do |filename|
      ...
      if File.exist? file_install_path
        next if ask("#{file_install_path} already exists. Do you want to overwrite?", %w[y n]) == 'n'
      else
